...
### Task Importance Convention

Tasks use `+` symbols for importance in daily notes. During `/cco-process-daily` extraction, CC converts to a queryable inline field:

**In daily note:** `- [ ] Fix token refresh +++++`
**In task file:** `- [ ] Fix token refresh [importance:: 5] — [[2026-02-09 • PeerLoop]]`

More `+` = more important. No cap, but practically 1-5. Due dates are optional (only when there's a real deadline). No delegation/assignee fields.
...
### 🕒 Timecard • ⚽️ Coding • 07:00 to 09:00
...
3. For each qualifying heading:
   ...
   e. Extracts tasks into per-project task files, converting `+++++` to `[importance:: 5]`
...
- [ ] Investigate token refresh timing ⏫ 📅 2026-02-20 — [[2026-02-09 • PeerLoop]]
- [ ] Draft dark mode proposal — [[2026-02-09 • PeerLoop]]
...
## Tasks
- [ ] Are there LLM / Models that are better at translations? [importance:: 2]
```

**Notes:**
- Simpler than coding cards — no Git History, no For Client/For Admin, no machine.
- Tasks section holds any action items (extracted to project task file during processing).
...
**Body sections (video/in-person meeting):**

```markdown
## Discussion
- Brian said PL will be invite-only at the beginning
- Gabriel suggested Puppeteer for multi-device testing via MCP server
- Discussed R2 vs Vimeo/YouTube — bandwidth costs vs storage costs

## Action Items
- [ ] Look into Puppeteer MCP server [importance:: 3]
- [ ] Research video hosting cost comparison [importance:: 2]

## Transcript
(if available)
```

**Body sections (phone call):**
...
**Body sections (site visit):**

```markdown
## Reason
- Quarterly check-in and equipment review
...
- `record` indicates who is recording or taking notes.
- Phone calls and site visits add a `## Reason` section before Discussion.
- Action Items / Tasks section extracted to project task files during processing.
...
## Tasks
- [ ] Decide: Standardize file naming for tests [importance:: 3]
```
...
- Simplified Section 7 (Warranties) — posts become another SOW/block
- Refined Section 10 (Liability) cap language
- Document attached for lawyer review

## from: Brian — 10:30
- Confirmed receipt
- Forwarding to lawyer for review
- Expects feedback by end of week

## Attachments
- (attachment references)

## Tasks
- [ ] Follow up on lawyer feedback [importance:: 3]
```
...
8. **Task metadata:** Importance via `+` symbols (converted to `[importance:: N]` during extraction). No delegation. Due dates optional (rare). No urgency field — urgent work gets scheduled.
