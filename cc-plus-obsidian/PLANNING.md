...
| `Slack` | slack | From `/r-slack` comms skill |
| `Email` | email | From `/r-email` comms skill |
...
- PeerLoop alpha will be invite-only
#### Transcript
(from Otter.ai — inserted by /r-otter when record field is filled in)
#### Tasks
- [ ] Look into Puppeteer MCP server [importance:: 3]
```
...
- `record` indicates who is recording or taking notes. When present, `#### Transcript` section may be added by `/r-otter`.
...
**Created by:** `/r-slack` comms skill (from screenshot/paste) → human editing → extracted by `/cco-process-daily`

**Daily note heading:** `### 🕒 Slack • {channel} • HH:MM` *(format may evolve when comms skills are migrated)*
...
**Created by:** `/r-email` comms skill (from Gmail) → human editing → extracted by `/cco-process-daily`

**Daily note heading:** `### 🕒 Email • {person} • HH:MM` *(format may evolve when comms skills are migrated)*
